...
### CSS 布局:侧栏漂移到底部(grid 子元素数 > 行数)
**症状**:AI 侧栏不在右侧,跑到主内容下方,右侧空白。
**根因**:`.body` grid 有 3 个子元素(main/resizer/aside)但 `grid-template-rows: 1fr` 只定义 1 行。resizer 作为第二个子元素自动占了 col2/row1,把 aside 挤到 row2(横跨全宽)。
**修法**:resizer 改 `position: absolute`(不占 grid 单元格);main 显式 `grid-column:1`;aside 显式 `grid-column:2`。再加 `.app { height:100vh; overflow:hidden }` 锁视口,防止高内容(DAG 画布)撑破 grid 行高。
**诊断技巧**:CSS 渲染问题别盲改——用 playwright 抓 `getBoundingClientRect()` + `getComputedStyle().gridTemplateColumns`,一行数字立刻定位(`asideTop:2896` / `aside.left:0` 说明被挤到第二行)。
